...
const data = {
  title: "Class 12 Interactive Multiple Choice Questions (MCQs)",
  description:
  ...
  index: true,
  follow: true,
  image: "/mcqs_point/class_12_mcqs.webp",
...
export default function Page() {
  return (
    <SimpleWrapper data={data}>
      <Heading2 variant="black">Class 12 Mcqs</Heading2>
      <div className="grid gap-4 md:grid-cols-2 grid-cols-1 my-5">
        {mainData.map((book: { id: number; title: string; link: string }) => (
          <CardSmall
            key={book.id}
            title={book.title}
            link={`/class-12/mcqs/${book.link}/`}
          />
        ))}
      </div>
    </SimpleWrapper>
