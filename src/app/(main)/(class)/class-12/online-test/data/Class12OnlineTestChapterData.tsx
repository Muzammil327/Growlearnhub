...
export function Class12OnlineTestChapterData({ slug }: { slug: string }) {
  return (
      ...
      {data.map((data: any) => {
        return (
          <CardSmall
            key={data.name}
            title={data.name}
            link={`/class-12/online-test/${slug}/${data.slug}`}
          />
        )
      })}
    </div>
...
export function Class12OnlineTestChemistryChapterData({
  slug
}: {
  slug: string
}) {
