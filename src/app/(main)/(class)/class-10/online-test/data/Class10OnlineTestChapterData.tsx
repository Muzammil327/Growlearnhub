...
export function Class10OnlineTestChapterData({ slug }: { slug: string }) {
  return (
      ...
      {data.map((data: any) => {
        return (
          <CardSmall
            key={data.name}
            title={data.name}
            link={`/class-10/online-test/${slug}/${data.slug}`}
          />
        )
      })}
    </div>
...
export function Class10OnlineTestChemistryChapterData({
  slug
}: {
  slug: string
}) {
