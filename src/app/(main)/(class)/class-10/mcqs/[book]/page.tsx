...
export default async function page({ params }: PageProps) {
  ...
  return (
    <SimpleWrapper data={{
      title: "Class 10 " + SlugRemoveDashAndUppercase + " " + "Mcqs",
      canonical: `/class-10/mcqs/${slug}`,
      image: image,
      url: `https://growlearnhub.com/class-10/mcqs/${slug}`,
    }}>
...
export async function generateMetadata({ params }: PageProps) {
  const { book } = await params;
  const slug = book
  let url = `${process.env.NEXT_PUBLIC_SITE_URL}/class-10/mcqs/${slug}/`;
  let canonical = `/class-10/mcqs/${slug}/`;
  ...
  try {
    ...
    let title = "Class 10 " + SlugRemoveDashAndUppercase + " " + "Mcqs";
    let description = "Class 10" + " " + SlugRemoveDashAndUppercase + " " + "MCQs page offering a wide range of practice questions, online tests, and detailed answers for thorough exam preparation.";
    let keywords = [
