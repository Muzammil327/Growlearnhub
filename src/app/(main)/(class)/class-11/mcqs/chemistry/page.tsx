...
const data = {
  title: "Class 11 Chemistry Multiple Choice Questions (MCQs)",
  description:
    "Enhance your understanding of key Class 11 chemistry mcqs like motion, forces, energy, and waves. This resource aids in exam preparation.",
  index: true,
  follow: true,
...
export default function page() {
  return (
    <SimpleWrapper data={data}>
      <Heading2 variant="black">Class 11 Chemistry Mcqs</Heading2>
      <div className="grid gap-4 md:grid-cols-2 grid-cols-1 my-5">
        {mainData.map((book: { id: number; title: string; link: string }) => (
          <CardSmall
            key={book.id}
            title={book.title}
            link={`/class-11/mcqs/chemistry/${book.link}/`}
          />
        ))}
      </div>
    </SimpleWrapper>
