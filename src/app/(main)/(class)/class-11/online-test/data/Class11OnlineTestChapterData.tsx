...
export function Class11OnlineTestChapterData({ slug }: { slug: string }) {
  return (
      ...
      {data.map((data: any) => {
        return (
          <CardSmall
            key={data.name}
            title={data.name}
            link={`/class-11/online-test/${slug}/${data.slug}`}
          />
        )
      })}
    </div>
...
export function Class11OnlineTestChemistryChapterData({
  slug
}: {
  slug: string
}) {
