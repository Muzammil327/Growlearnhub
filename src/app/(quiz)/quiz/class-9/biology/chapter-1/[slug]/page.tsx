...
import { DecodeSlug } from "@/src/functions/slugify";
...
export default function Page(params: {
  ...
  const slugs = params.params.slug;
  const title = DecodeSlug(slugs);
  const data4 = class9_biology_chapter1_quiz.filter(
  ...
  return (
    ...
    >
      <div className="my-5">
        <div className="mb-3">
          <h3>
            <Link href={`${data5.title}`}>
              {data5.id}. {data5.title}
            </Link>
          </h3>
          <ol type="A">
            {data5.options.map((data: any) => (
              <li key={data}>{data}</li>
            ))}
          </ol>
          <span className="mb-3">
            The Correct options is <strong>{data5.correct_options}</strong>.
          </span>
        </div>
      </div>
